...
Param(
    ...
    [ValidateSet("Test", "Major", "Minor", "Patch")]
    [String] $Type,
...
Try {
    ...
    [Object[]] $OriginalModules = Get-Module
    If ($null -eq $OriginalModules) {
        $OriginalModules = @()
    }

    Import-Module "$PSScriptRoot\modules\Paths.psm1"
    Initialize-Paths -ScriptRoot $PSScriptRoot -ModName $ModName | Out-Null
    ...
    # folder
    #
    Copy-Item -Path (Get-ModSourceFile) -Destination (Get-ReleaseModSourceFile)
    ...
    #
    Set-Content -Path (Get-VersionFile) -Value $NewVersion
